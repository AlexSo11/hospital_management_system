...
class DBMedicamentos:
    """Clase para manejar operaciones de base de datos de medicamentos - PostgreSQL"""
    ...
    def insertar_medicamento(datos):
        ...
        try:
            ...
            # Insertar medicamento con RETURNING (PostgreSQL)
            query = """
    ...
    def obtener_medicamentos_proximos_vencer(dias=30):
        ...
        try:
            ...
            # PostgreSQL usa CURRENT_DATE e INTERVAL
            query = """
                SELECT id_medicamento, nombre, via_administracion, 
                       presentacion, fecha_caducidad,
                ...
                AND fecha_caducidad <= CURRENT_DATE + INTERVAL '%s days'
            ...
            for med in medicamentos:
                medicamentos_list.append({
                    ...
                    'presentacion': med[3],
                    'fecha_caducidad': med[4],
                    'dias_restantes': med[5].days if hasattr(med[5], 'days') else med[5]
                })
    ...
    def obtener_medicamentos_vencidos():
        ...
        try:
            ...
            query = """
                SELECT id_medicamento, nombre, via_administracion, 
                       presentacion, fecha_caducidad,
                       (CURRENT_DATE - fecha_caducidad) as dias_vencido
                FROM medicamentos
                WHERE activo = TRUE 
                AND fecha_caducidad < CURRENT_DATE
            ...
            for med in medicamentos:
                medicamentos_list.append({
                    ...
                    'presentacion': med[3],
                    'fecha_caducidad': med[4],
                    'dias_vencido': med[5].days if hasattr(med[5], 'days') else med[5]
                })
