...
from DB_XAMPP.dbDoctores import DBDoctor
from datetime import datetime

class UIDoctores:
    def __init__(self, parent):
        self.ventana = tk.Toplevel(parent)
        self.ventana.title("Gestión de Doctores - Sistema Hospitalario")
    ...
    def crear_widgets(self):
        ...
        tk.Label(
            header_frame,
            text="Sistema de Registro y Consulta de Doctores",
            font=("Arial", 10),
            bg='#0077be',
            fg='#e0f0ff'
        ).pack(pady=(0, 10))
        ...
        # Pestaña 1: Insertar Doctor
        self.frame_insertar = tk.Frame(self.notebook, bg='#ffffff', relief='flat', bd=1)
        self.notebook.add(self.frame_insertar, text="Registrar Doctor")
        ...
        self.notebook.add(self.frame_consultas, text="Listado de Doctores")
        self.crear_tabla_consultas()
        
    def crear_formulario_insertar(self):
        """Crea el formulario para insertar doctor con nuevo diseño"""
        ...
        crear_campo(right_frame, "Especialidad", self.combo_especialidad, True)
    ...
    def crear_tabla_consultas(self):
        """Crea la tabla para mostrar todos los doctores con nuevo diseño"""
        # Frame principal
        ...
        titulo = tk.Label(
            main_frame,
            text="Listado de Doctores Registrados",
            font=("Arial", 16, "bold"),
            bg='#ffffff',
            fg='#0077be'
        )
        ...
        self.tree_doctores = ttk.Treeview(
            tabla_frame,
            columns=("ID", "Nombre", "Dirección", "Teléfono", "F. Nacimiento", 
                     "Sexo", "Especialidad", "Usuario"),
            show="headings",
        ...
        scroll_y.config(command=self.tree_doctores.yview)
        scroll_x.config(command=self.tree_doctores.xview)
        ...
        for col, ancho in columnas.items():
            self.tree_doctores.heading(col, text=col)
            self.tree_doctores.column(col, width=ancho, anchor=tk.CENTER)
        
        # Empaquetar todo
        self.tree_doctores.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
    ...
    def guardar_doctor(self):
        ...
        # Insertar en BD
        exito, mensaje, id_doctor = DBDoctor.insertar_doctor(datos)
        
        if exito:
            messagebox.showinfo("Éxito", f"{mensaje}\nID asignado: {id_doctor}")
            self.limpiar_campos()
            self.cargar_doctores()  # Actualizar tabla
        else:
    ...
    def validar_campos(self):
        ...
        if not self.combo_especialidad.get().strip():
            messagebox.showwarning("Advertencia", "La especialidad es obligatoria")
            self.combo_especialidad.focus()
            return False
    ...
    def cargar_doctores(self):
        """Carga todos los doctores en la tabla"""
        # Limpiar tabla
        for item in self.tree_doctores.get_children():
            self.tree_doctores.delete(item)
        
        # Obtener doctores de la BD
        exito, resultado = DBDoctor.obtener_todos_doctores()
        
        if exito:
            for doc in resultado:
                self.tree_doctores.insert("", tk.END, values=(
                    doc['id_doctor'],
                    doc['nombre'],
                    doc['direccion'],
                    doc['telefono'],
            ...
            self.lbl_info.config(text=f"Total de doctores: {len(resultado)}")
        else:
